...
import { checkPlan, nodesFromPlan, renderPlanMarkdown } from "./plan.js";
import { Subtree, TIER_DEFAULTS } from "./schema.js";
...
const TIERS = ["focused", "standard", "deep"];
function depth(plan, id) {
    let d = 0;
    for (let cur = plan.nodes.find((n) => n.id === id); cur?.parent; cur = plan.nodes.find((n) => n.id === cur.parent))
        d++;
    return d;
}
export async function proposeRedecomposition(store, runId, nodeId, input) {
    ...
    return withRunLock(store, id, async () => {
        ...
        if (node.kind !== "leaf")
            throw new GraftreeError(`${nodeId} is already a split; re-decompose one of its leaves`, "invalid");
        if (node.status === "done")
            throw new GraftreeError(`${nodeId} already has a winner; nothing to re-decompose`, "invalid");
        if (!input.reason.trim())
            throw new GraftreeError("redecompose needs --reason (why the leaf could not be solved whole)", "invalid");
        const used = run.redecompositions.filter((r) => r.node === nodeId).length;
        if (used >= cfg.budgets.maxRedecompositions) {
            throw new GraftreeError(`${nodeId} was already re-decomposed ${used} time(s) (budgets.maxRedecompositions = ${cfg.budgets.maxRedecompositions}); raise the budget or start a new run`, "budget");
        }
        const parsed = Subtree.safeParse(input.subtree);
        if (!parsed.success)
            return { run, plan: null, errors: [`subtree does not match schema:\n${formatIssues(parsed.error.issues)}`], warnings: [] };
        const sub = parsed.data;
        const clash = sub.nodes.filter((n) => run.nodes[n.id]).map((n) => n.id);
        if (clash.length)
            return { run, plan: null, errors: [`node id(s) already in the plan: ${clash.join(", ")}`], warnings: [] };
        ...
        if (input.testsFrom && !existsSync(input.testsFrom))
        ...
        for (const f of files) {
            if (locked.has(f))
                throw new GraftreeError(`${f} is a locked acceptance test; re-decomposition only adds new files`, "invalid");
            const exists = await git(store.root, ["cat-file", "-e", `${run.approval.baseCommit}:${f}`]).then(() => true, () => false);
            if (exists)
                throw new GraftreeError(`${f} already exists in the run base; re-decomposition only adds new files`, "invalid");
        }
        ...
        target.kind = "split";
        if (sub.sharedPaths)
            target.sharedPaths = sub.sharedPaths;
        ...
        if (existsSync(store.testsDir(id)))
            await cp(store.testsDir(id), staged, { recursive: true });
        if (input.testsFrom)
            await cp(input.testsFrom, staged, { recursive: true });
...
export async function approveRedecomposition(store, runId, notes) {
    const id = (await store.loadRun(runId)).id;
    return withRunLock(store, id, async () => {
        const run = await store.loadRun(id);
        const p = run.pendingRedecomposition;
        if (!p || run.status !== "awaiting_approval" || !run.approval)
            throw new GraftreeError(`run ${id} has no pending re-decomposition`, "bad_status");
        const staged = join(store.runDir(id), p.stagedTests);
        const oldNode = run.nodes[p.node];
        let baseCommit = run.approval.baseCommit;
        if (p.files.length) {
            ({ commit: baseCommit } = await commitOverlay(store.root, p.files.map((f) => ({ repoPath: f, sourcePath: join(staged, f) })), `graftree: re-decomposition tests for ${id} ${p.node}\n\n${p.reason}`, run.approval.baseRef, run.approval.baseCommit));
            for (const f of p.files)
                run.approval.locked.push({ path: f, sha256: await sha256File(join(staged, f)) });
        ...
        for (const a of oldNode.attempts)
            if (a.worktree)
...
export async function rejectRedecomposition(store, runId, notes) {
    const id = (await store.loadRun(runId)).id;
    return withRunLock(store, id, async () => {
        const run = await store.loadRun(id);
        const p = run.pendingRedecomposition;

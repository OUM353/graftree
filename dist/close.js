 ...
 */
export async function closeRun(store, runId, opts = {}) {
...
async function closeLocked(store, id, opts) {
    ...
    try {
        ...
        if (cfg.commands.lint)
            await check("lint", cfg.commands.lint);
    }
    finally {
        await removeWorktree(store.root, wt);
    }
...
async function renderReport(store, run, cfg, checks, commit) {
    const plan = run.plan;
    const md = [];
    ...
    if (run.final)
        md.push(`**Result:** branch \`${run.final.branch}\` (${run.final.commit.slice(0, 12)}): ${stat.files} files, +${stat.insertions} −${stat.deletions} vs. your HEAD at approval  `);
    ...
    for (const [k, v] of Object.entries(checks))
        md.push(`| \`${k}\` | ${v.ok ? "✅ pass" : `❌ exit ${v.exitCode}`} |`);
    md.push("");
    md.push("## Nodes", "");

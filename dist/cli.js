...
function reportPlanCheck(out, run, check, store, extra = {}) {
    if (!check.plan) {
    ...
    print(out, `Plan accepted for ${run.id} — status: awaiting_approval\n\n${renderTree(check.plan)}${warn}\n\nReview: ${store.planMdPath(run.id)}\nThen:  graftree approve ${run.id}   or   graftree reject ${run.id} --notes "…"`, { ok: true, run: run.id, status: "awaiting_approval", planMd: store.planMdPath(run.id), warnings: check.warnings, ...extra });
    return 0;
}
function attemptNo(s) {
    const n = Number(s.replace(/^a/i, ""));
    if (!Number.isInteger(n) || n < 1)
        throw new GraftreeError(`invalid attempt "${s}" (use 2 or a2)`, "invalid");
    return n;
}
function humanSummary(sum) {
...
async function main(argv) {
    ...
    switch (cmd) {
        ...
        case "plan": {
            ...
            if (values.worker && values.file)
                throw new GraftreeError("use either --file or --worker, not both", "invalid");
            if (values.worker) {
        ...
        case "show": {
            ...
            if (run.plan) {
                lines.push("", renderTree(run.plan, (n) => {
            ...
            if (run.approval)
                lines.push(`Approved ${run.approval.approvedAt}; base ${run.approval.baseRef} = ${run.approval.baseCommit.slice(0, 12)}; ${run.approval.locked.length} test file(s) locked`);
        ...
        case "diff": {
            const [node, att] = rest;
            if (!node || !att)
                throw new GraftreeError("usage: graftree diff NODE ATTEMPT [--run R]", "invalid");
        ...
        case "decide": {
            const [node, att] = rest;
            if (!node || !att)
                throw new GraftreeError("usage: graftree decide NODE ATTEMPT [--run R] [--notes …]", "invalid");
        ...
        case "attempt": {
            const [node] = rest;
            if (!node || (!values.worktree && !values.commit))
                throw new GraftreeError("usage: graftree attempt NODE (--worktree PATH | --commit REV) [--run R]", "invalid");
        ...
        case "close": {
            ...
            print(out, res.ok
                ...
                : `Final checks FAILED for ${res.run.id}:\n${lines.join("\n")}\nReport: ${res.report}`, { ok: res.ok, run: res.run.id, status: res.run.status, final: res.run.final, checks: res.checks, report: res.report });
            return res.ok ? 0 : 5;
        }
        case "clean": {
            const run = await store.loadRun(rest[0]);
        ...
        case "lock-check": {
            if (!values.commit)
                throw new GraftreeError("lock-check needs --commit REV", "invalid");
            const run = await store.loadRun(rest[0]);
            const violations = await checkLocked(store, run, values.commit);
            print(out, violations.length ? `DISQUALIFIED — locked tests changed:\n${violations.map((v) => `  ${v.reason}: ${v.path}`).join("\n")}` : "OK — locked tests untouched.", { ok: violations.length === 0, violations });
            return violations.length ? 3 : 0;
        }
        case "worker": {
            if (rest[0] !== "test" || !rest[1])
                throw new GraftreeError('usage: graftree worker test NAME ["prompt"]', "invalid");
            ...
            const res = await runWorker(rest[1], w, { prompt, cwd: scratch, timeoutSec: 300 }, "complete").finally(() => rm(scratch, { recursive: true, force: true }));
            print(out, `${res.ok ? "✓" : "✗"} ${res.worker} (exit ${res.exitCode}${res.timedOut ? ", timed out" : ""}, ${res.durationMs} ms)\n${res.text || res.stderr}`, { ok: res.ok, exitCode: res.exitCode, timedOut: res.timedOut, durationMs: res.durationMs, text: res.text, stderr: res.stderr, usage: res.usage });
            return res.ok ? 0 : 1;
        }
        default:

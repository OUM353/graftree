...
import { CLOSER, Config } from "./schema.js";
...
function checkRoleReferences(cfg) {
...
export function workersForRole(cfg, role) {
    return cfg.roles[role];
}
export function getWorker(cfg, name) {
    const w = cfg.workers[name];
    if (!w)
        throw new GraftreeError(`unknown worker "${name}" (defined: ${Object.keys(cfg.workers).join(", ") || "none"})`, "config");
    return w;
}

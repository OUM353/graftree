...
export function parseNpmCmdShim(text: string, shimDir: string): string | null {
...
export function findOnPath(cmd: string, env: NodeJS.ProcessEnv, platform = process.platform): string | null {
  const win = platform === "win32";
  const exts = win ? (env.PATHEXT ?? ".COM;.EXE;.BAT;.CMD").split(";").filter(Boolean) : [];
  const variants = (base: string) => [base, ...exts.map((e) => base + e.toLowerCase())];
  if (isAbsolute(cmd) || cmd.includes("/") || cmd.includes("\\")) return variants(cmd).find((p) => existsSync(p)) ?? null;
...
export function resolveCommand(cmd: string, env: NodeJS.ProcessEnv = process.env, platform = process.platform): ResolvedCommand {
  if (platform !== "win32") return { file: cmd, prefixArgs: [] };
  const found = findOnPath(cmd, env, platform);
  if (!found) return { file: cmd, prefixArgs: [] };
  if (/\.(cmd|bat)$/i.test(found)) {
    const script = parseNpmCmdShim(readFileSync(found, "utf8"), dirname(found));
    if (script && existsSync(script)) return { file: process.execPath, prefixArgs: [script] };
    throw new Error(
      `${found} is a batch file graftree can't run safely (not an npm node shim). Point the worker's command at the underlying executable or script instead.`,
    );
  }

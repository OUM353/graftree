...
if (ref) {
  scratch = mkdtempSync(join(tmpdir(), "tasklog-grade-"));
  dir = join(scratch, "wt");
...
try {
  ...
  const r = spawnSync(process.execPath, ["--test", "--test-reporter=tap", join("holdout", "tasklog.holdout.test.mjs")], { cwd: dir, env, encoding: "utf8" });
  const out = r.stdout ?? "";
